...
def make_parser():
    ...
    """
    parser = argparse.ArgumentParser(description='%s: run all or some tests for the SuiteSparse.py library' % os.path.basename(sys.argv[0]))
    parser.add_argument("-r", "--rebuild", help="Rebuild from scratch the SuiteSparse.py library", action='store_true', required=False)
    parser.add_argument("-b", "--build", help="Build (if needed) SuiteSparse.py library with new code", action='store_true', required=False)
